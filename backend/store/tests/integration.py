...
def main():
    ...
    try:
        sync_resp = requests.post(
            f"{BASE_URL}/api/stores/sync/", json=sync_payload, headers=headers
        )
    ...
    time.sleep(
        2
    )  # Wait a bit for celery to maybe pick it up (though in dev it might be fast)

    try:
        status_resp = requests.get(
            f"{BASE_URL}/api/stores/sync/status/", headers=headers
        )
        ...
        if status_data["products_count"] != 1 or status_data["faqs_count"] != 1:
            print(
                "⚠️ Warning: Counts do not match what we just synced. Make sure Celery worker is running."
            )

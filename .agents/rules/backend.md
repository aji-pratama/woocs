...
- Domain apps are `store`, `chat`, and `billing`; shared infrastructure lives in `common`.
...
| `billing` | Store-owned Subscription projection, Polar checkout/portal, signed webhook reconciliation |

## 3. Models
- All domain models use **UUID primary keys** (`uuid.uuid4`).
...
## 6. Background tasks
- Use Django 6 Tasks with the custom PostgreSQL backend in `common`.
- Keep tasks in `<app>/tasks.py` and enqueue heavy work instead of blocking requests.

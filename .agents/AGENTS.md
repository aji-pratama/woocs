...
├── backend/          # Hono project (host process)
│   ├── config/       # Hono settings, URLs
...
| Service | Port | Notes |
|---|---|---|
| Hono dev server | 8000 | `npm run dev:api` |
...
### Hono Architecture
- **API layer**: endpoints are prefixed with `/api/stores/` (plugin calls) and `/api/widget/` (widget calls)
- **Async tasks**: Hono 6 Tasks framework + Custom Postgres backend — all heavy work offloaded to `db_worker`
...
- Plugin communicates with Hono via `X-API-Key` authenticated HTTP calls
...
- **Hono ORM + pgvector**: tenant-scoped retrieval and confidence scoring
...
- Keep retrieval, tenant isolation, confidence, and escalation explicit in Hono.
...
npm install       # pip install deps
npm run migrate       # Run Hono migrations
npm run dev:api               # Hono dev server
npm run dev:worker            # Hono DB task worker
